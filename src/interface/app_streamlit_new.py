...
from run_batch_ux import get_agent_app
        ...
        with st.expander("⚙️ Configuración del Agente"):
            ...
            if proveedor == "En línea (Gemini)":
                ...
                try:
                    ...
                    modelos_disponibles = [m.model for m in modelos_info.models] if hasattr(modelos_info, 'models') else [m['name'] for m in modelos_info.get('models', [])]
                except:
                    modelos_disponibles = ["Error al conectar con Ollama"]
                selected_model = st.selectbox("Modelo local:", modelos_disponibles)

                # selected_model = st.selectbox("Modelo Local", ["mistral:latest", "llama3:latest"])
